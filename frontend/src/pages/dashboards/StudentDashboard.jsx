  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 rounded-xl p-6 text-white">
        <h1 className="text-2xl font-bold mb-2">Welcome Back, {stats?.user?.name}!</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card, index) => (
          <div key={index} className="bg-white rounded-xl shadow-sm p-6">
            ...
            {card.link && (
              <Link to={card.link} className="text-sm text-primary-600 hover:text-primary-700 font-medium mt-3 inline-block">
                View All →
              </Link>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            ...
            {(stats?.attendance?.percentage || 0) < 75 && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mt-3">
                <p className="text-sm text-red-800 font-medium">⚠️ Below 75% requirement</p>
              </div>
            )}
          </div>
          <Link
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">Marks Overview</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <span className="font-medium text-gray-700">Submit New Activity</span>
          </Link>
          <Link
          ...
          >
            ...
            <span className="font-medium text-gray-700">View Portfolio</span>
          </Link>
          <Link
            to="/student/attendance"
            className="flex items-center justify-center p-4 border-2 border-dashed border-gray-300 rounded-lg hover:border-primary-500 hover:bg-primary-50 transition"
